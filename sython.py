...
ownerhson_id = 6563583299
@sython.on(events.NewMessage(outgoing=False, pattern='X1'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply('@nnxex')
...
- سورس بسيط يحتوي على الاوامر المهمة وتم اضافتله الكثير من اليوزرات العربيه والصينيه والمميزه
BY : @NNXEX
